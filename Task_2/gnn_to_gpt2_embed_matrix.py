...
def make_amr_graph_lists(data_file):
    ...
    for i, g in enumerate(graphs):
        nx_graph = nx.DiGraph()
        out_text=prompt_text+g.metadata.get('snt', None)
...
amr_graphs_test, text_list_test,amr_string_list_test= make_amr_graph_lists('qald-9-amr-test.txt')
model_path = 'gpt2'
...
class GNNtoGPT2(nn.Module):
    ...
    def forward(self, node_indices, edge_index, edge_type, input_ids, attention_mask,text_ids,text_mask,batch,mask):
        ...
        full_embedding=torch.concat((graph_embedding,text_embedding),dim=1)
        ...
        outputs = self.gpt2_model(input_ids=input_ids, attention_mask=attention_mask,encoder_hidden_states=full_embedding)
        return outputs
...
def test(model, test_loader, gpt2_tokenizer, file_path, max_length=100):
    """
    Test the model using batched greedy decoding for text generation.
    """
...
best_model_path=train(model,loader,optimizer,criterion,"small_amr_text_graph_conct")
gnn_encoder = GNNEncoder(num_nodes=len(node_to_idx), embedding_dim=128, hidden_dim=hidden_dim, output_dim=768, num_relations=num_relations)
text_encoder=LinearizedGraphEnc(768)
model = GNNtoGPT2(gnn_encoder,text_encoder, gpt2_model, 768)
...
model.to(DEVICE)
test(model,testloader,gpt2_tokenizer,file_path="small_amr_text_graph_conct.csv")
