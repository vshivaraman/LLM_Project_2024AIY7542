    ...
    with torch.no_grad():  # Disable gradient computation for testing
        for batch in test_loader:
            ...
            for _ in range(max_length):
                ...
                # Perform batched nucleus sampling
                probs = torch.softmax(logits, dim=-1)
                sorted_probs, sorted_indices = torch.sort(probs, descending=True, dim=-1)
                cumulative_probs = torch.cumsum(sorted_probs, dim=-1)

                # Create cutoff mask for nucleus sampling
                cutoff_mask = cumulative_probs > p
                cutoff_mask[..., 1:] = cutoff_mask[..., :-1].clone()
                cutoff_mask[..., 0] = False  # Always keep the first token in the nucleus
                sorted_probs[cutoff_mask] = 0.0
                sorted_probs = sorted_probs / sorted_probs.sum(dim=-1, keepdim=True)  # Renormalize probabilities

                # Sample the next tokens for the entire batch
                next_tokens = torch.multinomial(sorted_probs, num_samples=1).squeeze(-1)
                next_indices = sorted_indices.gather(dim=-1, index=next_tokens.unsqueeze(-1)).squeeze(-1)

                # Stop generation for sequences that generate an EOS token
                if (next_indices == gpt2_tokenizer.eos_token_id).all():
                    break

                # Append generated tokens and update input for the next step
                for i, token in enumerate(next_indices.tolist()):
                    if token != gpt2_tokenizer.eos_token_id:
                        generated_tokens[i].append(token)

                input_ids = torch.cat([input_ids, next_indices.unsqueeze(-1)], dim=-1)
                attention_mask = torch.cat([attention_mask, torch.ones_like(next_indices).unsqueeze(-1)], dim=-1)
...
# best_model_pat=train(model,loader,optimizer,criterion,"small_amr_mha_align_graph_text")
gnn_encoder = GNNEncoder(num_nodes=len(node_to_idx), embedding_dim=128, hidden_dim=hidden_dim, output_dim=768, num_relations=num_relations)
text_encoder=LinearizedGraphEnc(768)
model = GNNtoGPT2(gnn_encoder,text_encoder, gpt2_model, 768)
...
model.to(DEVICE)
test(model,testloader,gpt2_tokenizer,file_path="small_amr_mha_align_graph_text.csv")
# test_with_beam_search(model,testloader,gpt2_tokenizer,file_path="small_amr_mha_align_graph_text.csv")
